...
static void  _attempt_limits(void);
...
/*
 * _attempt_limits -  Determine if jobs exceeded their overall CPUhours limit.
 *     Reading example.cfg for user's limit list.
 */
static void _attempt_limits(void)
{

  	info("_attempt_limits");
  	config_t cfg;
  ...
  /* Если конфиг неправильный будем по умолчанию все изменять.   */
	if(! config_read_file(&cfg, "/opt/slurm/etc/example.cfg"))
  	{
  	...
  	setting = config_lookup(&cfg, "limits.users");
  	if(setting != NULL)
  	{
		info("inside if");
    ...
    //Подсчет количество процессорочасов уже поставленных задач
    for(i = 0; i < count; ++i)
    {
	info("inside for\n");
	config_setting_t *listt = config_setting_get_elem(setting, i);
      	//slurmctld_lock_t job_write_lock = {READ_LOCK, WRITE_LOCK, WRITE_LOCK, READ_LOCK}; 
	const char *user;
      	uint32_t limit;
	uid_t userid;
	List job_queue1;
	...
	//проверить 
      	if(!(config_setting_lookup_string(listt, "user", &user)
           && config_setting_lookup_int(listt, "limit", &limit))) {
		info("problems with file reading\n");        	
		continue;
      	}
	 ...
	 if(user) {
			...
			info("%-30s  %-30s  %-30s", "USER", "USERID", "LIMIT");	
			info("%-30s  %-30d  %-30d\n", user, userid, limit);
			...
			/*
			job_queue = build_job_queue(true, true); //job_list
			...
			*/
			...
			//while ((job_queue_rec = (job_queue_rec_t *) list_pop(job_queue))) {
				...
				info("%-30s  %-30s  %-30s", "USER", "JOBUSERID", "USEDLIMIT");	
				info("%-30s  %-30u  %-30d\n", user, job_ptr->user_id, job_ptr->time_limit);

				info("JobId=%u. State=%s. Reason=%s. Priority=%u. Partition=%s.",job_ptr->job_id,job_state_string(job_ptr->job_state),job_reason_string(job_ptr->state_reason),job_ptr->priority,job_ptr->partition);
				//info("%-30d %-30d \n", userid, job_ptr->user_id);
		
				if((job_ptr->user_id == userid) && (job_ptr->job_state != JOB_CANCELLED)){

					if (job_ptr->time_limit == INFINITE)
						time_limit = time_limit + 365 * 24 * 60 * 60;
					else if (job_ptr->time_limit != NO_VAL)
						time_limit = time_limit + job_ptr->time_limit * 60;
					else if (job_ptr->part_ptr && (job_ptr->part_ptr->max_time != INFINITE))
						time_limit = time_limit + job_ptr->part_ptr->max_time * 60;
					else
						time_limit = time_limit + 365 * 24 * 60 * 60;
			
					if((time_limit > limit) && (job_ptr->job_state == JOB_PENDING)) {//add "job_ptr->state_reason == @none@" //&& (job_ptr->state_reason != WAIT_RESOURCES)
						info("Changing state");

						info("%-30s  %-30s  %-30s  %-30s ", "USER", "JOBID", "USEDLIMIT", "time_limit");	
						info("%-30s  %-30u  %-30u  %-30d\n", user, job_ptr->job_id, job_ptr->time_limit, time_limit);
						info("JobId=%u. State=%s. Reason=%s. Priority=%u. Partition=%s.",job_ptr->job_id,job_state_string(job_ptr->job_state),job_reason_string(job_ptr->state_reason),job_ptr->priority,job_ptr->partition);
						job_ptr->job_state = JOB_SUSPENDED;
						job_ptr->state_reason = WAIT_QOS_TIME_LIMIT; // WAIT_RESOURCES;
						info("JobId=%u. State=%s. Reason=%s. Priority=%u. Partition=%s.",job_ptr->job_id,job_state_string(job_ptr->job_state),job_reason_string(job_ptr->state_reason),job_ptr->priority,job_ptr->partition);
				
					}			
					/*
					if((time_limit <= limit)&& (job_ptr->state_reason == WAIT_QOS_TIME_LIMIT)) // && (job_ptr->job_state == JOB_SUSPENDED)) //&& (job_ptr->state_reason == WAIT_RESOURCES) 
					{	
						//job_ptr->job_state = JOB_PENDING;
						info("Changing state back111");
						info("%-30s  %-30s  %-30s  %-30s ", "USER", "JOBID", "USEDLIMIT", "time_limit");	
						info("%-30s  %-30u  %-30u  %-30d\n", user, job_ptr->job_id, job_ptr->time_limit, time_limit);
						info("JobId=%u. State=%s. Reason=%s. Priority=%u. Partition=%s.",job_ptr->job_id,job_state_string(job_ptr->job_state),job_reason_string(job_ptr->state_reason),job_ptr->priority,job_ptr->partition);
					...
					*/
					if((time_limit <= limit)&& (job_ptr->state_reason == WAIT_QOS_TIME_LIMIT) && (job_ptr->job_state == JOB_SUSPENDED)) // && (job_ptr->job_state == JOB_SUSPENDED)) //&& (job_ptr->state_reason == WAIT_RESOURCES) 
					{	
						//job_ptr->job_state = JOB_PENDING;
						
						info("Changing state back");
						info("%-30s  %-30s  %-30s  %-30s ", "USER", "JOBID", "USEDLIMIT", "time_limit");	
						info("%-30s  %-30u  %-30u  %-30d\n", user, job_ptr->job_id, job_ptr->time_limit, time_limit);
						info("JobId=%u. State=%s. Reason=%s. Priority=%u. Partition=%s.",job_ptr->job_id,job_state_string(job_ptr->job_state),job_reason_string(job_ptr->state_reason),job_ptr->priority,job_ptr->partition);
						job_ptr->job_state = JOB_PENDING;
	...
	info("%-30s", "Out of custom function");
  	}}}
	
	ff:  config_destroy(&cfg);
}
...
static void _apply_limits(void)
{
	...
	uint32_t *uid = NULL, nuser = 5, bf_parts = 0, *bf_part_jobs = NULL;
	uint16_t *njobs = NULL;
